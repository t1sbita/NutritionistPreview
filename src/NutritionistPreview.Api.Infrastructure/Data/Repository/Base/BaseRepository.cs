...
namespace NutritionistPreview.Api.Infrastructure.Data.Repository.Base
{
    public class BaseRepository<T> : IDisposable, IBaseRepository<T> where T : BaseEntity
    {
        private bool _disposed;
        protected readonly NutritionistContext Context;
        private DbSet<T> Entities { get; set; }

        public BaseRepository(NutritionistContext contexto)
        {
            Context = contexto;
           ...
           params Expression<Func<T, object>>[] includes)
        {
            ...
            PageConsultation<T> pageConsultation = new();
